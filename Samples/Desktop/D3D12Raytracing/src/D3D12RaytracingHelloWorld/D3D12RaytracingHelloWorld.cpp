...
// number of triangles to display
const int numTri = 5;
...
void D3D12RaytracingHelloWorld::BuildAccelerationStructures()
{
    ...
    topLevelInputs.NumDescs = numTri;
    ...
    D3D12_RAYTRACING_INSTANCE_DESC instanceDesc[numTri] = {};
    for (int i = 0; i < numTri; i++)
    {
        // identity matrix
        instanceDesc[i].Transform[0][0] = instanceDesc[i].Transform[1][1] = instanceDesc[i].Transform[2][2] = 1;
        // translation
        instanceDesc[i].Transform[0][2] = instanceDesc[i].Transform[1][2] = -1 + (0.3f * (i+1));
